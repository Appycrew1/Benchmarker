...
app = FastAPI(title="Moving Benchmark + Heatmap — MVP Plus (No DB)")
...
COMP_PRICES = { k: [max(60, min(150, BASE[k]["rate"] + random.randint(-15, 15))) for _ in range(6)] for k in BASE.keys() }
AD_INTENSITY = { k: random.randint(30, 85) for k in BASE.keys() }

def intensity(d, c): return max(0.0, min(1.0, (d - c + 50) / 100.0))
def area_avg_price(code): return statistics.mean(COMP_PRICES.get(code, [BASE[code]["rate"]]))

@app.get("/api/areas")
def areas(): return AREAS

@app.get("/api/heatmap")
def heatmap(): return [[a["lat"], a["lng"], intensity(BASE[a["code"]]["demand"], BASE[a["code"]]["comp"])] for a in AREAS]

@app.get("/api/metrics")
def metrics(area_code: str = Query(...)):
    if area_code not in BASE: return {"error": "not found"}
    m = BASE[area_code]
    return {"area_code": area_code,"demand_index": m["demand"],"competition_index": m["comp"],"avg_hourly_rate": m["rate"],
            "avg_review_score": m["rev"],"close_rate_pct": m["close"],"on_time_pct": m["ont"],"avg_job_value": m["job"],
...
def benchmark(area_code: str, your_hourly_rate: float = 0.0, your_review_score: float = 0.0):
    data = metrics(area_code); if "error" in data: return data
    return {"area_code": area_code,"area_avg":{"hourly_rate": data["avg_hourly_rate"],"review_score": data["avg_review_score"],
...
# A) Forecast (30 days, weekly seasonality + trend + noise)
@app.get("/api/forecast")
def forecast(area_code: str, horizon_days: int = 30):
    if area_code not in BASE: return {"error":"not found"}
    base_d = BASE[area_code]["demand"]; out = []
    for i in range(horizon_days):
        seasonal = 5 * math.sin((i/7)*2*math.pi)
        trend = (i / 60.0) * 3
...
# B) Dynamic pricing based on demand/comp + competitor avg
@app.get("/api/pricing")
def pricing(area_code: str):
    if area_code not in BASE: return {"error":"not found"}
    m = BASE[area_code]; comp_avg = area_avg_price(area_code); demand, comp, your = m["demand"], m["comp"], m["rate"]
    pressure = (demand - comp) / 100.0
    target = comp_avg * (1 + 0.15 * pressure)
    delta_pct = (target - your) / your
    rec = {"current_rate": your,"competitor_avg_rate": round(comp_avg,1),"recommended_rate": round(target, 1),
           "change_pct": round(delta_pct * 100, 1),
           "rationale": "High demand and/or lower competition allows a premium; aligned towards competitor average." if pressure>0 else "Lower demand or higher competition suggests staying closer to competitor average."}
    return {"area_code": area_code, "pricing": rec}

# C) Competitor watch: simulate snapshot & alert on big change
...
def competitor_watch(area_code: str):
    ...
    prices2 = prices[1:] + [new_price]; COMP_PRICES[area_code] = prices2
    prev_avg = round(statistics.mean(prices[:-1]), 1); latest = prices2[-1]; delta = latest - prices[-1]
    alert = f"Competitor price {'decrease' if delta<0 else 'increase'} of £{abs(delta)} detected." if abs(delta) >= 10 else None
    return {"area_code": area_code,"history": prices2,"latest": latest,"prev_avg": prev_avg,"alert": alert}

# D) Lead score: combine job value + demand delta + your reviews
@app.get("/api/lead_score")
def lead_score(area_code: str, est_job_value: float = 500.0, your_review_score: float = 4.3):
    if area_code not in BASE: return {"error":"not found"}
    m = BASE[area_code]; v = min(1000.0, max(100.0, est_job_value))
    v_norm = (v - 100.0) / (900.0) * 100.0
...
# E) Marketing: rank by (demand - comp) minus ad intensity
@app.get("/api/marketing")
def marketing():
    ranked = []
    for a in AREAS:
        code = a["code"]; m = BASE[code]; ad = AD_INTENSITY[code]
        roi_proxy = (m["demand"] - m["comp"]) - (ad - 50)
        ranked.append({"area_code": code,"roi_proxy": round(roi_proxy,1),"suggested_action": "Increase" if roi_proxy>0 else "Reduce",
                       "note": f"Demand {m['demand']} vs Comp {m['comp']}; Ad intensity {ad}"})
